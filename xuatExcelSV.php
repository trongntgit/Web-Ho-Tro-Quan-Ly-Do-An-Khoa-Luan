...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $sheet->setCellValue("A$rowNumber", $row['ma'])
            ...
            ->setCellValue("C$rowNumber", $row['lop'])
            ->setCellValue("D$rowNumber", $row['diemtichluy'])
    ...
    $filename = 'Danh_sach_sinh_vien_' . time() . '.xlsx';
    ...
    echo json_encode(["success" => false, "message" => "Không có dữ liệu giảng viên để xuất"]);
}
?>
