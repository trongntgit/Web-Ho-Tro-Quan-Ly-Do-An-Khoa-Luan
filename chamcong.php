...
<head>
    ...
    <title>Chấm công Mobifone</title>
</head>
<body>
    <div class="main">
        <div class="body">
           ...
           <!-- Modal -->
            <div id="modal-nhanvien">
                <div id="modal-content">
                    <h2>Thêm nhân viên</h2>
                    <div id="modal-tabs">
                        <button class="tab-button" data-tab="select">Chọn nhân viên</button>
                        <button class="tab-button" data-tab="upload">Tải lên file Excel</button>
                    </div>
                    <div id="modal-body">
                        <!-- Tab 1: Chọn nhân viên -->
                        <div id="tab-select" class="tab-content">
                            <h3>Chọn nhân viên</h3>
                            <select name="op-nv" id="op-nv" style="height: 40px; width: 120px;">
                                <?php
                                    ...
                                    // Truy vấn SQL để lấy mã NV và họ tên từ bảng nhân viên
                                    $query = "SELECT manv, hoten FROM nhanvien";
                                    $result = mysqli_query($con, $query);

                                    // Hiển thị các tùy chọn
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        echo '<option value="' . $row['manv'] . '" data-hoten="' . $row['hoten'] . '">' . $row['manv'] . ' - ' . $row['hoten'] . '</option>';
                                    }
                            ...
                            <button id="add-nv">Thêm nhân viên</button>
                        </div>
                        <!-- Tab 2: Tải lên file Excel -->
                        <div id="tab-upload" class="tab-content">
                            <h3>Tải lên file Excel</h3>
                            <input type="file" id="file-upload" accept=".xls, .xlsx" />
                            <button id="upload-file">Tải lên</button>
                        </div>
                    </div>
                ...
                <div id="dialog">
                    ...
                    <button id="dialog-close">Đóng</button>
                </div>
            </div>
        </div>
        ...
        <div id="dia-camera" class="draggable resizable" style="display: none;">
    ...
    <script src="./assets/javas/chamcong.js"></script>
