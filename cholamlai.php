...
if (isset($_POST['madetai'])) {
    ...
    $sql = "UPDATE detai SET trangthai = 'Đang thực hiên' WHERE madetai = ?";
    ...
    try {
        ...
        // Nội dung thông báo
        $noidung = "Đề tài $madetai của bạn đã được giáo viên cho phép thục hiện lại, hãy cố gắng nhé !";
        ...
        $query_phieudiem = "DELETE FROM phieudiem where madetai = ? and magv = ?";
        $stmt_phieudiem = $conn->prepare($query_phieudiem);
        if ($stmt_phieudiem === false) {
            throw new Exception("Lỗi chuẩn bị câu lệnh xóa dữ liệu phieudiem: " . $conn->error);
        }
