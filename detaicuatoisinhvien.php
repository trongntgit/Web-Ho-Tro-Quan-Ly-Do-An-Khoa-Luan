...
$sql_all = "
    SELECT dangkydetai.*, detai.tendetai, detai.tengv
    FROM dangkydetai
    JOIN detai ON dangkydetai.madetai = detai.madetai
    WHERE dangkydetai.masv = ? AND (dangkydetai.trangthai = 'Chờ duyệt' OR dangkydetai.trangthai = 'Chấp nhận')
";
...
SELECT 
    ...
    detai.tendetai, 
    detai.tengv,
    COALESCE(SUM(tiendo.phantram), 0) AS phantram_tiendo
FROM 
    dangkydetai
JOIN 
    detai ON dangkydetai.madetai = detai.madetai
LEFT JOIN 
    tiendo ON dangkydetai.madetai = tiendo.madetai AND tiendo.loaitiendo = 'progress'
WHERE 
    dangkydetai.masv = ? 
    AND dangkydetai.trangthai = 'Chấp nhận'
    AND detai.trangthai = 'Đang thực hiện'
GROUP BY 
    dangkydetai.madetai, detai.tendetai, detai.tengv;
                ...
                <section class="project-lists">
                            ...
                            <?php
                                    if ($result_all->num_rows > 0) {
                                        while ($row = $result_all->fetch_assoc()) {
                                            ...
                                            // Kiểm tra trạng thái trước khi hiển thị nút
                                            if ($row['trangthai'] === "Chờ duyệt") {
                                                echo '<button style="width : 110px;" class="view-btn-huy" id="'.$row['madetai'].'" data-madt="' . $row['madetai'] . '" data-masv ="'.$manguoitao.'" onclick="huyDangKy(this)">Hủy đăng ký</button>';
                                            } else {
                                                echo ""; // Giữ trống nếu không phải "Chờ duyệt"
                                            }
                                        ...
                                        echo "<tr><td colspan='6'>Không có đề tài nào.</td></tr>";
                                    }
                                    ?>
                    ...
                    <div id="dialog-xemdk" class="modal">
                        <div class="modal-content">
                            ...
                            <button id="accept-btn" class="button" onclick="acceptRegistration()">Chấp nhận</button>
                            ...
                            <tbody>
                                ...
                                if ($result_in_progress->num_rows > 0) {
                                    while ($row = $result_in_progress->fetch_assoc()) {
                                        ...
                                        // echo "<button class='edit-btn'>Chỉnh sửa</button>";
                                        echo "</td>";
                                        echo "</tr>";
                                    }
                                } else {
                                    ...
                                    if ($result_completed->num_rows > 0) {
                                        while ($row = $result_completed->fetch_assoc()) {
                                            ...
                                            if ($trangthai === 'Không đạt') {
                                                echo "<span class='fail-message'>Không thể mở hành động</span>";
                                            } else {
                                                echo "<a class='view-btn-tiendo' href='./detai/" . $row['madetai'] . ".php'>Xem chi tiết</a>";
                                            }
